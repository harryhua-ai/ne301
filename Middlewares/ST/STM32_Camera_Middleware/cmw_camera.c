...
/**
 * @brief  Set the auto exposure control (AEC) mode.
 * @param  enable Set to 1 to enable AEC or 0 to disable.
 * @retval CMW status
 *
 * @note If the camera module does not support AEC control, the function returns CMW_ERROR_FEATURE_NOT_SUPPORTED.
 * @note If the enable argument is greater than 1, the function sets enable to 1.
 */
int CMW_CAMERA_SetAEC(uint32_t enable)
{
  int ret;
  if(Camera_Drv.SetAEC == NULL)
  {
    return CMW_ERROR_FEATURE_NOT_SUPPORTED;
  }
  ...
  ret = Camera_Drv.SetAEC(&camera_bsp, enable);
  if (ret != CMW_ERROR_NONE)
  {
    return CMW_ERROR_COMPONENT_FAILURE;
  }

  Camera_Ctx.AEC = enable;
  return CMW_ERROR_NONE;
}

/**
 * @brief  Get the current auto exposure control (AEC) mode.
 * @param  enable pointer to the current AEC mode (1: enabled, 0: disabled)
 * @retval CMW status
 */
int CMW_CAMERA_GetAEC(uint32_t *enable)
{
  *enable = Camera_Ctx.AEC;
  return CMW_ERROR_NONE;
}


/**
 * @brief  Set the camera contrast.
 * @param  Contrast     Contrast value, range dependent on the camera module.
 * @retval CMW status
 *
 * @note If the camera module does not support contrast control, the function returns CMW_ERROR_FEATURE_NOT_SUPPORTED.
 * @note If the enable argument is greater than the maximum value supported by the camera, the function sets enable to the maximum value supported by the camera.
 */
int CMW_CAMERA_SetContrast(int32_t Contrast)
{
  int ret;
  if(Camera_Drv.SetContrast == NULL)
  {
    return CMW_ERROR_FEATURE_NOT_SUPPORTED;
  }

  ret = Camera_Drv.SetContrast(&camera_bsp, Contrast);
  if (ret != CMW_ERROR_NONE)
  {
    return CMW_ERROR_COMPONENT_FAILURE;
  }

  Camera_Ctx.Contrast = Contrast;
  return CMW_ERROR_NONE;
}


/**
 * @brief  Get the current camera contrast value.
 * @param  Contrast pointer to the current camera contrast value.
 * @retval CMW status
 *
 * @note If the camera module does not support contrast control, the function returns CMW_ERROR_FEATURE_NOT_SUPPORTED.
 */
int CMW_CAMERA_GetContrast(int32_t *Contrast)
{
  *Contrast = Camera_Ctx.Contrast;
  return CMW_ERROR_NONE;
}
...
#if defined(USE_VD66GY_SENSOR) || defined(USE_IMX335_SENSOR) || defined(USE_OS04C10_SENSOR)
