...
ULONG                       packet_buffer_size;
    ...
    {   
        ...
        #else
            if (cdc_ecm->ux_host_class_cdc_ecm_link_up_callback)
            {
                cdc_ecm->ux_host_class_cdc_ecm_link_up_callback(cdc_ecm);
            } else {
                printf("link up\n");
            }
        #endif
                ...
                if (cdc_ecm -> ux_host_class_cdc_ecm_link_state == UX_HOST_CLASS_CDC_ECM_LINK_STATE_UP)
                {
                    ...
                    if (status == UX_SUCCESS)
                    {
                        ...
                        if (transfer_request -> ux_transfer_request_completion_code == UX_SUCCESS)
                        {
                            ...
                            /* Get the packet.  */
                            packet.nx_packet_ptr = transfer_request -> ux_transfer_request_data_pointer;

                            // TODO:
                            if (cdc_ecm->ux_host_class_cdc_ecm_receive_callback)
                            {
                                cdc_ecm->ux_host_class_cdc_ecm_receive_callback(cdc_ecm, &packet);
                            } else {
                                printf("packet received: %lu\n", packet.nx_packet_length);
                                if (packet.nx_packet_length > 0)
                                {
                                    for (int i = 0; i < packet.nx_packet_length; i++)
                                    {
                                        printf("%02X ", packet.nx_packet_ptr[i]);
                                        if (i % 16 == 15) printf("\n");
                                    }
                                    printf("\n");
                                }
        ...
        #else
            if (cdc_ecm->ux_host_class_cdc_ecm_link_down_callback)
            {
                cdc_ecm->ux_host_class_cdc_ecm_link_down_callback(cdc_ecm);
            } else {
                printf("link down\n");
            }
        #endif
