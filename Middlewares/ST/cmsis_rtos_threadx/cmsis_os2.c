...
osThreadId_t osThreadNew(osThreadFunc_t func, void *argument, const osThreadAttr_t *attr)
{
  ...
  if (!IS_IRQ_MODE() && (func != NULL))
  {
    ...
    else
    {
      ...
      /* Mark ownership flags using TX_THREAD_USER_EXTENSION field */
      {
        ULONG flags = 0;
        if (attr != NULL)
        {
          if (attr->stack_mem == NULL) { flags |= TX_THREAD_ALLOC_OWNS_STACK; }
          if (attr->cb_mem    == NULL) { flags |= TX_THREAD_ALLOC_OWNS_CB; }
        }
        else
        {
          /* No attributes: both allocated internally */
          flags |= (TX_THREAD_ALLOC_OWNS_STACK | TX_THREAD_ALLOC_OWNS_CB);
        }
        thread_ptr->tx_thread_alloc_flags = flags;
      }
    }
  }
...
osStatus_t osThreadTerminate(osThreadId_t thread_id)
{
  ...
  else
  {
    ...
    {
      /* Always delete to remove from created list and clean kernel resources */
      ...
      {
        /* Only free memory that was allocated by the wrapper itself */
        ULONG flags = thread_ptr->tx_thread_alloc_flags;
        if ((flags & TX_THREAD_ALLOC_OWNS_STACK) != 0U)
        {
          MemFree(thread_ptr->tx_thread_stack_start);
        }
        if ((flags & TX_THREAD_ALLOC_OWNS_CB) != 0U)
        {
          MemFree(thread_ptr);
        }
      }
