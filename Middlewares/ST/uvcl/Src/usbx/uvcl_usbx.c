...
VOID my_usbx_debug_callback(UCHAR *debug_message, ULONG debug_value)
{
    printf("[USBX] ");
    printf((const char *)debug_message, debug_value); // Formatted output
    printf("\n");
}
#endif
...
#ifdef UX_ENABLE_DEBUG_LOG
  _ux_utility_debug_callback_register(my_usbx_debug_callback);
#endif
