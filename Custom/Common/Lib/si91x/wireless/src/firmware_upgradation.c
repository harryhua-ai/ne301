                               ...
                               uint32_t post_data_length)
{
  ...
  sli_si91x_http_client_request_t *http_client   = SLI_MALLOC(sizeof(sli_si91x_http_client_request_t));
  ...
  if (flags & SL_SI91X_HTTPS_USE_SNI) {
    if (host_name == NULL) {
      SLI_FREE(http_client);
      return SL_STATUS_NULL_POINTER;
    }

    https_enable = (https_enable | SL_SI91X_HTTPS_USE_SNI);

    sl_si91x_socket_type_length_value_t *set_sni = (sl_si91x_socket_type_length_value_t *)SLI_MALLOC(
  ...
  if (http_length <= SLI_SI91X_MAX_HTTP_CHUNK_SIZE) {
    ...
    packet_buffer = SLI_MALLOC(sizeof(sli_si91x_http_client_request_t));
...
sl_status_t sl_si91x_http_otaf_v2(const sl_si91x_http_otaf_params_t *http_otaf_params)
{
  ...
  sl_status_t status = sl_si91x_http_otaf(0,
