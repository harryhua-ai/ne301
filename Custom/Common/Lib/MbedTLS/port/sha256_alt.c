...
/* Private typedef -------------------------------------------------------------*/
/* Private define --------------------------------------------------------------*/
#define ST_SHA256_TIMEOUT ((uint32_t) 1000)  /* TO in ms for the hash processor */
...
static mbedtls_threading_mutex_t hash_mutex;
static uint32_t hash_context_count = 0;
...
void mbedtls_sha256_init(mbedtls_sha256_context *ctx)
{
  ...
  if (hash_context_count == 0) mbedtls_mutex_init(&hash_mutex);
  hash_context_count++;
  __enable_irq();
#endif /* MBEDTLS_THREADING_C */
...
void mbedtls_sha256_free(mbedtls_sha256_context *ctx)
{
  ...
  if (hash_context_count > 0) {
    ...
    if (hash_context_count == 0)
    {
      ...
      if (ctx->hhash.Instance == HASH) HAL_HASH_DeInit(&ctx->hhash);
    }
  }
  __enable_irq();
#endif /* MBEDTLS_THREADING_C */
...
exit :
#if defined(MBEDTLS_THREADING_C)
  mbedtls_mutex_unlock(&hash_mutex);
#endif /* MBEDTLS_THREADING_C */
  return ret;
}
