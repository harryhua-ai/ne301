...
ULONG                   copied;
    ...
    if (cdc_ecm -> ux_host_class_cdc_ecm_link_state == UX_HOST_CLASS_CDC_ECM_LINK_STATE_UP)
    {
        ...
        memcpy(cdc_ecm -> ux_host_class_cdc_ecm_xmit_buffer, packet -> nx_packet_ptr, packet -> nx_packet_length);
        /* Setup the transaction parameters.  */
        transfer_request -> ux_transfer_request_data_pointer     = cdc_ecm -> ux_host_class_cdc_ecm_xmit_buffer;
        transfer_request -> ux_transfer_request_requested_length = packet -> nx_packet_length;
