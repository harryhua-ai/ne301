...
#error "incorrect EWL memory management configuration"
  ...
  ewl_instance.ewl_event_group = osEventFlagsNew(NULL);
...
i32 EWLRelease(const void *inst)
{
  ...
  err = tx_byte_pool_delete(&ewl_instance.byte_pool);
  if (err)
  {
  ...
  // osEventFlagsDelete(ewl_instance.ewl_event_group);
#else
#endif /* EWL_ALLOC_API */
...
__weak i32 EWLMallocLinear(const void *instance, u32 size, EWLLinearMem_t *info)
{
  ...
  __attribute__((unused)) VENC_EWL_TypeDef *inst = (VENC_EWL_TypeDef *) instance;
  ...
  // info->virtualAddress = malloc(size_aligned);
  info->virtualAddress = EWLmalloc(size_aligned);
#elif (EWL_ALLOC_API == EWL_USE_THREADX_MM)
...
#error "no STM32MPM yet"
#elif (EWL_ALLOC_API == EWL_USER_MM)
  info->virtualAddress = hal_mem_alloc_aligned(size_aligned, 32, MEM_LARGE);
#else
#error "incorrect EWL memory management configuration"
#endif /* EWL_ALLOC_API */
...
#elif (EWL_ALLOC_API == EWL_USE_FREERTOS_MM)
  EWLfree(info->virtualAddress);
#elif (EWL_ALLOC_API == EWL_USE_THREADX_MM)
  for (u32 i = 0; i < inst->totalChunks; i++)
  {
    if (inst->alignedChunks[i] == info->virtualAddress)
    {
      printf("EWLFreeLinear addr:0x%x\r\n", (int)info->virtualAddress);
...
#error "no STM32MPM yet"
#elif (EWL_ALLOC_API == EWL_USER_MM)
  hal_mem_free(info->virtualAddress);
#else
#error "incorrect EWL memory management configuration"
#endif /* EWL_ALLOC_API */
...
__weak i32 EWLWaitHwRdy(const void *inst, u32 *slicesReady)
{
  ...
  //   return EWL_HW_WAIT_TIMEOUT;
  // }
  uint32_t flags = osEventFlagsWait(ewl_instance.ewl_event_group, 1, osFlagsWaitAll, EWL_TIMEOUT);
  if ((flags & 1) == 0) // Event not received
  {
      return EWL_HW_WAIT_TIMEOUT;
  }
  ret = EWL_OK;
...
void VENC_IRQHandler(void)
{
  ...
  if(irq_status != 0U)
  {
    ...
    if(irq_status & ASIC_STATUS_HW_TIMEOUT){
      PTRACE("irq_status :0x%x\n",irq_status);
      // EncAsicStop(&ewl_instance);
    ...
    // }
    uint32_t result = osEventFlagsSet(ewl_instance.ewl_event_group, 1);
