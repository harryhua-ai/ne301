...
#include "factory_test.h"
                                              ...
                                              const json_config_validation_options_t *validation_options)
 {
     ...
     /* Import semantics: the device's CURRENT config is the merge base.
     * json_config_parse_json_object only overlays keys the JSON actually
     * contains, so anything the file omits (or the user deleted from an
     * exported file) keeps the device's own value instead of reverting to
     * factory defaults — and the subsequent set_config full-save then
     * persists the merged values, never clobbering untouched fields. */
    if (g_json_config_ctx.initialized)
    {
 ...
 aicam_result_t json_config_reset_to_default(const char *file_path)
 {
     ...
     /* Factory-burned MAC is authoritative: when present it overrides the
      * runtime-captured value here, and netif_init_manager programs it onto
      * the WiFi AP interface (device_service never takes the AP address
      * over it either). */
     uint8_t factory_mac[6];
     if (factory_mac_get_burned(factory_mac) == 0)
         snprintf(preserved_info.mac_address, sizeof(preserved_info.mac_address),
                  NETIF_MAC_STR_FMT, NETIF_MAC_PARAMETER(factory_mac));
...
aicam_result_t json_config_set_capture_upload_config(const capture_upload_config_t *config)
{
    ...
    json_config_capture_upload_normalize(&norm);
