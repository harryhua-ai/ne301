...
void ll_aton_reloc_log_info(const uintptr_t file_ptr)
{
  ...
  {
    AI_RELOC_LOG(" %d: flags=%lx foff=%ld dst=%lx s=%ld %s\r\n", index, (unsigned long)mem_c_desc->flags, (long)mem_c_desc->foff, (unsigned long)mem_c_desc->dst,
                 (long)mem_c_desc->size, (char *)AI_RELOC_GET_ADDR(bin, (uint32_t)mem_c_desc->name));
    index++;
  }
  ...
  AI_RELOC_LOG("                  fpu=%d (is_enabled:%d)\r\n", (int)AI_RELOC_RT_FPU_USED(rt_info.variant),
               fpu_is_enabled);
  AI_RELOC_LOG("                  cpuid=0x%03lx (0x%03lx)\r\n", (unsigned long)AI_RELOC_RT_GET_CPUID(rt_info.variant),
               (unsigned long)_GET_PART_NUMBER());
