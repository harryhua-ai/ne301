...
/* Forward declaration for convert_itoa if not available from header */
#ifndef SLI_SI91X_INTERNAL_HTTP_CLIENT
  ...
  if (client_internal->configuration.https_use_sni) {
    ...
    if (request->sni_extension != NULL) {
      ...
      if (request->host_name != NULL && request->ip_address != request->host_name) {
        ...
        sl_si91x_socket_type_length_value_t *tls_sni = (sl_si91x_socket_type_length_value_t *)SLI_MALLOC(
