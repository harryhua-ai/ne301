...
void* ATC_Malloc(size_t size)
{
  ...
  ptr = (void*)memalign(64, size); 
#elif ATC_RTOS == ATC_RTOS_THREADX

#elif ATC_RTOS == ATC_RTOS_USER
  ptr = hal_mem_alloc_aligned(size, 64, MEM_LARGE);
#endif
  return ptr;
}
...
bool ATC_TxRaw(ATC_HandleTypeDef* hAtc, const uint8_t* Data, uint16_t Len)
{
  ...
  do
  {
    ...
    osDelay(15);
    // HAL_UART_Transmit(hAtc->hUart, Data, Len,  ~0);
    answer = true;
...
void ATC_CheckErrors(ATC_HandleTypeDef* hAtc)
{
  if (HAL_UART_GetError(hAtc->hUart) != HAL_UART_ERROR_NONE)
  {
    ...
    SCB_InvalidateDCache_by_Addr((uint32_t*)hAtc->pRxBuff, hAtc->Size);
...
#if ATC_RTOS == ATC_RTOS_DISABLE
  HAL_Delay(Delay);
#elif (ATC_RTOS == ATC_RTOS_CMSIS_V1) || (ATC_RTOS == ATC_RTOS_CMSIS_V2) || (ATC_RTOS == ATC_RTOS_USER)
