            ...
            if (status == UX_SUCCESS)
            {
                ...
                if (status == UX_SUCCESS)
                {
                    ...
                    if (status == UX_SUCCESS)
                    {
                        ...
                        #else
                            if (cdc_ecm->ux_host_class_cdc_ecm_activate_callback)
                            {
                                cdc_ecm->ux_host_class_cdc_ecm_activate_callback(cdc_ecm, cdc_ecm->ux_host_class_cdc_ecm_node_id);
                            } else {
                                printf("activate, mac = %02x:%02x:%02x:%02x:%02x:%02x\n", 
                                    cdc_ecm -> ux_host_class_cdc_ecm_node_id[0], cdc_ecm -> ux_host_class_cdc_ecm_node_id[1], 
                                    cdc_ecm -> ux_host_class_cdc_ecm_node_id[2], cdc_ecm -> ux_host_class_cdc_ecm_node_id[3], 
                                    cdc_ecm -> ux_host_class_cdc_ecm_node_id[4], cdc_ecm -> ux_host_class_cdc_ecm_node_id[5]);
                            }
                        #endif
