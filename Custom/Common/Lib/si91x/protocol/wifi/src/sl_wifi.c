...
sl_status_t sl_wifi_wait_for_scan_results(sl_wifi_scan_result_t **scan_results, uint32_t max_scan_result_count)
{
  ...
  if (packet->command == SLI_WLAN_RSP_SCAN) {
    *scan_results = (sl_wifi_scan_result_t *)SLI_MALLOC(packet->length);
...
#define SLI_MAX_GAIN_TABLE_SIZE_WITH_TB_ONLY 128
  ...
  sli_si91x_gain_table_info_t *sl_gain_table_info = SLI_MALLOC(sizeof(sli_si91x_gain_table_info_t) + su_tb_payload_length);
                                         ...
                                         uint8_t y_offset)
{
  ...
  sli_si91x_gain_table_info_t *sl_gain_table_info = SLI_MALLOC(sizeof(sli_si91x_gain_table_info_t) + payload_length);
  if (sl_gain_table_info == NULL) {
    return SL_STATUS_ALLOCATION_FAILED;
  }
