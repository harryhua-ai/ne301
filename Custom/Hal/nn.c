...
{
    nn_t *nn = (nn_t *)handle;
    if (!nn || nn->state != NN_STATE_READY || !classes) {
        LOG_DRV_ERROR("model not loaded or classes is NULL\r\r\n");
        return -1;
    }
...
int nn_get_parsed_class_list(nn_class_list_t *classes)
{
    ...
    return nn_instance_get_parsed_class_list(g_nn_single_instance, classes);
}
