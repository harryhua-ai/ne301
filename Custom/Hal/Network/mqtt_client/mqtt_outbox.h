...
#define OBX_MEM_CHECK(a, action) if (!(a)) {                                        \
        LOG_DRV_ERROR("%s(%d): %s",  __FUNCTION__, __LINE__, "Memory exhausted");   \
        action;                                                                     \
        }
...
typedef uint32_t outbox_tick_t;
...
int outbox_delete_item(outbox_handle_t outbox, outbox_item_handle_t item);
